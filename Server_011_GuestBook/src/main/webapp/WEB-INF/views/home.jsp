...
<style>
	...
	*/
	...
	.throth-text {
		/* 두께, 스타일, 색상, 모양 */
		/* dotted wavy solid dashed */
		text-decoration : 10px line-through red dashed; 
	}
	...
	<table id="gblist">
		...
		<c:forEach items="${GBLIST}" var="GB">
			<tr data-seq="${GB.gb_seq}">
				<c:set var="throth_text" value="${(GB.gb_seq % 2) == 0 ? 'throth-text' : '' }"/>
				<td class="${throth_text}">${GB.gb_date}</td>
				<td class="${throth_text}">${GB.gb_time}</td>
				<td class="${throth_text}">${GB.gb_writer}</td>
			</tr>
		</c:forEach>
	</table>
